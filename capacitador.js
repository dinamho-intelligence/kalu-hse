  ...
  function pintar() {
    ...
    el.innerHTML = `<div class="kc-wide">
      ...
      ${I ? `<div class="kc-kpis">${(I.indicadores || []).map(x => {
        ...
        const ok  = hay && Number(x.valor) >= Number(I.meta);
        return `<div class="kc-kpix ${hay ? (ok ? 'ok' : 'no') : 'nd'}">
          ...
          <div class="me"><i style="width:${hay ? Math.min(100, x.valor) : 0}%"></i>
            <u style="left:${Math.min(100, Number(I.meta))}%"></u></div>
          <div class="e">${hay
            ? `<span class="kc-tag ${ok ? 'si' : 'no'}">${ok ? '✓ cumple' : '! no cumple'}</span>
               <span>meta ${I.meta}%</span>`
            : `<span class="kc-tag n">— sin calcular</span> <span>meta ${I.meta}%</span>`}</div>
          <div class="f">${x.num} de ${x.den} ${esc(x.unidad)}</div>
          <div class="f2">${esc(x.formula)}</div>
        </div>`; }).join('')}</div>
      <p class="kc-nota" style="text-align:left;margin:-4px 0 16px">Metas tomadas del programa
        de capacitación de la empresa. Un indicador sin denominador dice «sin datos» y no 0%:
        no se puede calcular, y presentarlo en rojo sería inventar un incumplimiento.</p>` : ''}
